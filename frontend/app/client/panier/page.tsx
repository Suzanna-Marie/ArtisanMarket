...
export default function PagePanier() {
  const { articles, retirerArticle, modifierQuantite, total, nbArticles } = usePanierStore()
  const { user } = useAuthStore()
  const router = useRouter()

  const handlePayer = () => {
    if (!user) { router.push('/connexion'); return }
    router.push('/client/paiement')
  }
      ...
      <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="flex flex-col items-end justify-between">
                ...
                <div className="flex items-center border border-creme-fonce rounded-xl overflow-hidden">
                  <button
                    onClick={() =>
                      article.quantite > 1
                        ? modifierQuantite(article.produitId, article.quantite - 1)
                        : retirerArticle(article.produitId)
                    }
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-2xl border border-creme-fonce p-6 sticky top-20">
            ...
            <button onClick={handlePayer} className="w-full btn-primaire flex items-center justify-center gap-2">
              Payer avec Mobile Money
            </button>
            <p className="text-xs text-center text-muted-foreground mt-3">
              MTN · Moov · Celtiis — sécurisé par KKiaPay
            </p>
          </div>
        </div>
      </div>
