...
export const envoyerEmailValidationArtisan = async (email: string, prenom: string, nomBoutique: string) => {
  await transporter.sendMail({
    ...
    to: email,
    subject: '✅ Votre boutique ArtisanMarket est validée !',
    html: emailBase(`
...
export const envoyerEmailRejetArtisan = async (email: string, prenom: string, nomBoutique: string, motif?: string) => {
  await transporter.sendMail({
    from: process.env.FROM_EMAIL || `"ArtisanMarket" <${process.env.SMTP_USER}>`,
    to: email,
    subject: "Votre demande d'inscription ArtisanMarket",
    html: emailBase(`
      <p style="color:#1A1A1A;font-size:16px;margin:0 0 8px;">Bonjour <strong>${prenom}</strong>,</p>
      <p style="color:#555;font-size:15px;margin:0 0 24px;line-height:1.6;">
...
export const envoyerEmailSuspensionArtisan = async (email: string, prenom: string, nomBoutique: string) => {
  await transporter.sendMail({
    from: process.env.FROM_EMAIL || `"ArtisanMarket" <${process.env.SMTP_USER}>`,
    to: email,
    subject: 'Votre boutique ArtisanMarket a été suspendue',
    html: emailBase(`
      <p style="color:#1A1A1A;font-size:16px;margin:0 0 8px;">Bonjour <strong>${prenom}</strong>,</p>
      <p style="color:#555;font-size:15px;margin:0 0 24px;line-height:1.6;">
...
export const envoyerCodeReinitialisation = async (email: string, prenom: string, code: string) => {
  await transporter.sendMail({
    from: process.env.FROM_EMAIL || `"ArtisanMarket" <${process.env.SMTP_USER}>`,
    to: email,
    subject: `${code} - Réinitialisation de votre mot de passe ArtisanMarket`,
    html: `
<!DOCTYPE html>
<html lang="fr">
<head>
          ...
          <tr>
            <td style="background-color:#2D5016;padding:32px 40px;text-align:center;">
              <h1 style="margin:0;color:#ffffff;font-size:24px;font-weight:bold;">
                <span style="color:#E8B84B;">Artisan</span>Market
              </h1>
              <p style="margin:6px 0 0;color:rgba(255,255,255,0.7);font-size:13px;">
                La plateforme de l'artisanat béninois
              </p>
            </td>
          </tr>
          <tr>
            <td style="padding:40px;">
              ...
              <div style="text-align:center;margin:0 0 32px;">
                <div style="display:inline-block;background:#FAF7F0;border:2px solid #8B6914;border-radius:12px;padding:20px 40px;">
                  <p style="margin:0;font-size:40px;font-weight:bold;letter-spacing:12px;color:#8B6914;font-family:monospace;">
                    ${code}
                  </p>
                </div>
          ...
          <tr>
            <td style="background:#f5f0e8;padding:20px 40px;text-align:center;border-top:1px solid #e8e0d0;">
              <p style="margin:0;color:#aaa;font-size:12px;">
                © 2026 ArtisanMarket · Plateforme de l'artisanat du Bénin
              </p>
            </td>
          </tr>
        </table>
...
export const envoyerEmailContact_confirmation = async (nom: string, email: string) => {
  await transporter.sendMail({
    from: process.env.FROM_EMAIL || `"ArtisanMarket" <${process.env.SMTP_USER}>`,
    to: email,
    subject: 'Votre message a bien été reçu — ArtisanMarket',
    html: emailBase(`
...
export const envoyerCodeVerification = async (email: string, prenom: string, code: string) => {
  await transporter.sendMail({
    from: process.env.FROM_EMAIL || `"ArtisanMarket" <${process.env.SMTP_USER}>`,
    to: email,
    subject: `${code} - Votre code de vérification ArtisanMarket`,
    html: `
<!DOCTYPE html>
<html lang="fr">
<head>
